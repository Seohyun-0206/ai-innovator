...
└── src/
    ...
    └── models/
        ...
        ├── openai_model.py  # OpenAI-compatible API 클라이언트
...
OpenAI-compatible 엔드포인트라면 `OPENAI_BASE_URL`만 바꿔서 다른 제공자도 사용할 수 있다.
...
|---|---|---|
| `--model` | 필수 | 모델명 |
| `--num-questions` | 100 | 총 평가 문항 수 |
| `--num-shots` | 5 | few-shot 예시 수 |
...
# OpenAI-compatible 커스텀 엔드포인트 (환경변수 방식)
OPENAI_BASE_URL=https://my-endpoint/v1 OPENAI_API_KEY=xxx python main.py --model my-model

# 로컬 모델 (Ollama) — 별도 설정 불필요
...
| 모델명 접두사 | 라우팅 |
|---|---|
| `llama`, `mistral`, `gemma`, `qwen`, `phi`, `deepseek` 등 | Ollama (로컬) |
| 그 외 | OpenAI-compatible API |

로컬 모델 접두사 목록은 `config.py`의 `LOCAL_MODEL_PREFIXES`에서 추가/수정할 수 있다.
...
CSV 파일을 직접 다운로드해서 아래 구조로 배치해야 한다.

### 다운로드

[MMLU GitHub](https://github.com/hendrycks/test)에서 데이터를 받거나, 아래 명령어로 직접 다운로드한다.

```bash
git clone https://github.com/hendrycks/test.git mmlu-raw
cp -r mmlu-raw/data/test data/test
cp -r mmlu-raw/data/dev  data/dev
```
...
| 0 | 문제 |
| 1~4 | 선택지 A, B, C, D |
| 5 | 정답 인덱스 (0=A, 1=B, 2=C, 3=D) |

### 샘플링 방식

- STEM / Humanities / Social Sciences / Other 4개 카테고리 균등 샘플링
- 기본 100문항 (카테고리당 25문항)
